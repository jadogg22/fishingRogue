...
public partial class Fish : Area2D
{
	...
	[Export] public float PersonalSpace { get; set; } = 80.0f;
	...
	private void UpdateAggroMovement(float delta)
	{
		...
		{
			force += directionToHook.Normalized() * AggroSpeed;
			...
			if (parent != null)
			{
				...
				{
					if (other is Fish otherFish && otherFish != this && otherFish.Visible)
					{
						var dist = GlobalPosition.DistanceTo(otherFish.GlobalPosition);
						if (dist < PersonalSpace && dist > 0.0f)
						{
							var push = (GlobalPosition - otherFish.GlobalPosition).Normalized();
							// Stronger push the closer they are
							force += push * (AggroSpeed * 1.5f * (1.0f - dist / PersonalSpace));
						}
					}
				}
			}
		...
		{
			force += new Vector2(_currentSpeed * Direction, 0.0f);
		}

		Position += force * delta;

		// Constrain to water area
		Position = new Vector2(
			Mathf.Clamp(Position.X, -150.0f, 1430.0f),
