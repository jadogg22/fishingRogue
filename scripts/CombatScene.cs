...
public partial class CombatScene : Control
{
	...
	public void SetupRound(int currentPlayerHp, int maxPlayerHp, int currentBossHp, List<CardData> cardData, Array<Dictionary> playerStatuses, Array<Dictionary> bossStatuses, BossDefinition bossDefinition, int combatRoundNumber, int gold)
	{
		...
		var intentLabel = GetNode<Label>("VBox_MainLayout/Row2_PlayerArea/Margin_Player/VBox_PlayerStats/Label_Intent");
		intentLabel.Text = _enemyState == null ? "Boss Intent: -" : $"Boss Intent: {_enemyState.CurrentIntent.Description}";
		FxHelper.PulseControl(intentLabel, 1.05f, 0.25f);
	...
	private void UpdatePlayerUi()
	{
		...
		FxHelper.TweenProgressBar(playerBar, _playerState.CurrentHp);
	...
	private void UpdateEnemyUi()
	{
		...
		FxHelper.TweenProgressBar(bossBar, _enemyState.CurrentHp);
	...
	{
		var bossAnchor = GetNode<Control>("VBox_MainLayout/Row1_BossArea/BossSpriteAnchor");
		var bossPosition = bossAnchor.GlobalPosition;

		if (damage > 0)
		{
			FxHelper.ShakeControl(bossAnchor);
	...
	{
		var playerAnchor = GetNode<Control>("VBox_MainLayout/Row2_PlayerArea/PlayerSpriteAnchor");
		var playerPosition = playerAnchor.GlobalPosition;

		FxHelper.ShakeControl(playerAnchor, heavy ? 12.0f : 8.0f);
